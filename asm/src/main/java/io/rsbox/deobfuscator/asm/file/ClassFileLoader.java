...
public class ClassFileLoader implements ClassInfoLoader {
	...
	private Map<String, ZipFile> openZipFiles; // zip files to search for class files
	 ...
	 */
	public ClassInfo loadClassFromStream(final File inputFile,
	...
	public ClassInfo loadClass(String name) throws ClassNotFoundException {
		...
		SEARCH: while (end >= 0) {
			...
			if (f.isDirectory()) {
				...
				// Maybe a zip file?
				try {
					ZipFile zip = openZipFiles.get(dir);
